...
def calculate_score(dict_student, dict_answer):
    if len(dict_student) == len(dict_answer):
        for question,answer in dict_student.items():
            grade=0
            for question,answer in dict_answer.items():
                if answer == dict_student[question]:
                    grade+=1
        return grade
    else:
        return -1
